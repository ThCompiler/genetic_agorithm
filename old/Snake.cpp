...
namespace game
{
const base::count_t Snake::number_of_input_neuron = 5;
...
void Snake::move(const sf::Vector2f& feed, long height, long width) {
    ...
    for (size_t i = 1; i < _cubes.size(); ++i) {
...
move_target Snake::_think(const sf::Vector2f& feed, long height, long width) const {
    std::vector<bs::neuron_t> outputs = _agent.get_result(_create_inputs(feed, height, width));
