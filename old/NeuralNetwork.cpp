...
NN::NN(base::count_t number_inputs, std::vector<base::count_t> topology,
       base::count_t number_outputs, base::seed_t seed, genetic::Genotype genotype)
    : _number_inputs(number_inputs)
    , _number_outputs(number_outputs)
    , _topology(topology) {
    base::count_t number_weight = calculate_number_weigth(number_inputs, topology, number_outputs);

    if (genotype.is_zero()) {
        genotype.create_random_genotype(number_weight, defualt_min_parametr, defualt_max_parametr, seed);
    }
    ...
    _layers.push_back(NeuronalLayer(_topology[0], _number_inputs, seed));
    for (size_t i = 1; i < _topology.size(); ++i) {
        _layers.push_back(NeuronalLayer(_topology[i], _topology[i - 1], seed));
    }
    _layers.push_back(NeuronalLayer(_number_outputs, _topology.back(), seed));

    _set_genotype(genotype);
}

std::vector<neuron_t> NN::get_outputs(const std::vector<neuron_t>& inputs) const {
...
base::count_t NN::calculate_number_weigth(base::count_t number_inputs, std::vector<base::count_t> topology, base::count_t number_outputs) {
    base::count_t sum = number_inputs * topology[0] + number_outputs * topology.back();
...
void NN::set_activate_function(activation_func_t activate_function) {
